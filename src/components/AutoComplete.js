...
export const Search = ({ data }) => {
	...
	useEffect(() => {
		...
		updateFilteredResults(data.filter(result => result.title.toLowerCase().includes('')).length === 0 && data);
		console.log(filteredResults, searchResults);
	}, [data]);

	const updateSearch = e => {
		updateSearchTerm(e.target.value);
		updateFilteredResults(searchResults.filter(result => result.title.toLowerCase().includes(e.target.value)));
	};
			...
			<ul className="search-suggestions">
				...
				{!displayResults && filteredResults.map((article, index) => (
					<li key={index} className={focusIndex === index ? 'active' : null}>
						<a href={article.link} className="autosuggest-link"
